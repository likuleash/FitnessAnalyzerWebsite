...
# Convert Rows_Score to categories (bins)
num_classes = 3  # Number of classes for classification
data['Rows_Score_Category'] = pd.qcut(
    data['Rows_Score'],
    q=num_classes,
    labels=[f'Class_{i+1}' for i in range(num_classes)]
)

# Prepare Features (X) and Target (y)
X = data.drop(columns=['Rows_Score', 'Rows_Score_Category']).values  # Features (drop old target)
y = data['Rows_Score_Category'].values  # Target (binned labels)
